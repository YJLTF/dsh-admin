 ...
 *
 * 与其他桌面模块一致：IIFE + 'use strict'，依赖经
 * initSharedConfigEditor({ api, onSaved }) 注入（编辑器全部经
 * DOM API 写值，无 HTML 拼插，因此不需要 esc），返回 { loadShared }。
 */
;(function () {
  'use strict'

  function initSharedConfigEditor(ctx) {
    var api = ctx.api
    var onSaved = ctx.onSaved
    var provBox = document.getElementById('sharedProviders')
    var credBody = document.querySelector('#sharedCreds tbody')
    ...
    function credRow(ref = '', key = '') {
      ...
      refCell.appendChild(textInput(ref, '例如 SHARED_DEEPSEEK_KEY'))
    ...
    async function loadShared() {
      const r = await api('/api/admin/shared-config')
      if (!r.ok) {
    ...
    document.getElementById('saveSharedBtn').addEventListener('click', (event) => {
      const msg = document.getElementById('sharedMsg')
      window.DshCommon.setMsg(msg, '', false)
      ...
      void window.DshCommon.withBusy(event.currentTarget, async () => {
        const r = await api('/api/admin/shared-config', {
          method: 'PUT',
          body: JSON.stringify({ payload: { providers, credentials } }),
        })
        if (r.ok) {
          window.DshCommon.setMsg(msg, '已保存（v' + r.body.version + '）；已接收的用户会看到更新提示', false)
          await loadShared()
          if (onSaved) await onSaved()
        } else {
          window.DshCommon.setMsg(msg, '保存失败：' + (r.message || r.error || r.status), true)
        }
      })
    })
  ...
  window.initSharedConfigEditor = initSharedConfigEditor
})()
