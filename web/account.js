...
;(function () {
  ...
  function initAccountPanel(ctx) {
    ...
    function setPwMsg(text, isError) {
      window.DshCommon.setMsg($('pwMsg'), text, isError)
    }
    ...
    async function loadAccount() {
      ...
      for (var i = 0; i < r.body.sessions.length; i++) {
        ...
        tbody.insertAdjacentHTML(
          'beforeend',
          '<tr><td>' + esc(deviceLabel(s.userAgent)) + (isCurrent ? ' <span class="badge active">本机</span>' : '') + '</td><td>' + esc(s.ip || '—') + '</td><td>' + fmtDateTime(s.lastUsedAt) + '</td><td>' + fmtDateTime(s.expiresAt) + '</td><td>' + action + '</td></tr>',
        )
      }
