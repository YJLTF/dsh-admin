...
> 本教程面向 Linux 上的账号级硬隔离（Docker 容器内或裸机均可，均需以 root 运行编排服务）。
> `/root/dsh-admin` 换成你实际 `git clone` 的目录。
...
如果你用 systemd 跑编排服务（`/etc/systemd/system/dsh-admin.service`），默认就是 root，跳过这步。

---

## 2. 打开账号级隔离的开关

在环境变量文件里加两个配置（`/etc/dsh-admin.env`）：

```sh
# 追加这两行
DSH_ADMIN_ISOLATION_MODE=account
DSH_ADMIN_BASE_UID=100000
```
...
# 3) 环境变量指向系统 dsh
#    /etc/dsh-admin.env → DSH_ADMIN_DSH_BIN=/usr/local/bin/dsh

# 4) systemd 的 PATH 去掉 /root/.nvm/...，改成系统路径
#    （子 DSH 的 #!/usr/bin/env node 才找得到系统 node）
#    /etc/systemd/system/dsh-admin.service：
...
> **另一个降权后读不到的东西**：运行时插件 `dsh-admin/runtime` 在 `git clone` 的目录（如 `/dsh_login/dsh-admin/lib/`）里，降权账号也要能读到。检查 `ls -ld /dsh_login`，如果是 `drwx------`（只有 root），执行 `chmod 755 /dsh_login`。
...
chown -R "$uid:$uid" "/var/lib/dsh-admin/users/$1"
...
**userId 从哪拿**：用管理员登录 → 管理台 → 用户列表里那个 id（或直接查库：`sqlite3 <db> 'select id, username from users'`）。
...
[Path]
PathExistsGlob=/var/lib/dsh-admin/users/*/home
...
#!/usr/bin/env bash
set -euo pipefail
for dir in /var/lib/dsh-admin/users/*/; do
  [ -d "$dir" ] || continue
  id="$(basename "$dir")"
  user="dsh-$id"
  if ! id "$user" &>/dev/null; then
    # 账号不存在才建（uid 用插件算出来的同一个值，保证两边一致）
    uid="$(dsh-admin uid-for-user "$id")"
    useradd -u "$uid" -M -s /usr/sbin/nologin "$user"
  fi
...
> ⚠️ **别写「账号已存在就跳过」**：那样会连 chown 一起跳过——如果某用户的目录后来变成 root 所有，重跑也不会修，他的 DSH 还是会因为读不了自己 home 而崩（浏览器侧 502）。所以 chown 要放在判断**外面**、每次都执行。
...
**应该看到**：`uid` 等于 `dsh-admin uid-for-user <那个userId>`（是一个 >100000 的账号），**不是** 0（root）。如果显示 0，说明没降权，回头检查第 2、3 步。

**第 2 步：越权读测试（最关键）。**

```sh
# 1) 在用户 A 的 home 里放个文件
echo "secret" > /var/lib/dsh-admin/users/<A的id>/home/s.txt
chmod 600 /var/lib/dsh-admin/users/<A的id>/home/s.txt
